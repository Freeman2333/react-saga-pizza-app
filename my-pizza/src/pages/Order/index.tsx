...
import { ConfirmOrder, Container } from './styles';

const Order = () => {
  const dispatch = useDispatch();

  const { dough, flavor, price, size, recommendation } = useSelector(
    (state: ApplicationState) => state.order
  );
  const { total } = useSelector((state: ApplicationState) => state.points);
  const { points } = useSelector(
    (state: ApplicationState) => state.recommendations.pizza
  );
  ...
  return (
    <Container>
      ...
      <ConfirmOrder>
        ...
        <li>{flavor} </li>
        <li>
          <strong>TOTAL</strong> <span>R$ {price},00</span>
        </li>
      </ConfirmOrder>
      <AlignButton justify="space-between">
        ...
        <Link to="/finished">
          <Button icon="✔" version="secondary" onClick={finishOrder}>
